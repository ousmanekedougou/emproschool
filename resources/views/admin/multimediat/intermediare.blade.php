...
@section('main-content')
    ...
    <section class="content">
      ...
      <div class="">
        <div class="">
          <h3 class="box-title">Multimediat Niveau Intermediare</h3>
          ...
          <div class="tab-content">
            <div class="active tab-pane" id="activity">
              <table id="example1" class="table text-center table-bordered table-striped">
                ...
                <tbody>
                  @foreach($multimediat_intermediare as $multimediat_intermed)
                  <tr>
                  <td class="text-center">{{ $loop->index +1 }}</td>
                  <td class="text-center">
                    @if ($multimediat_intermed->genre == 1)
                       Femme
                    @elseif($multimediat_intermed->genre == 2)
                       Homme
                    @endif
                  </td>
                  <td class="text-center">{{ $multimediat_intermed->nomcomplet }}</td>
                  <td class="text-center">{{ $multimediat_intermed->email }}</td>
                  <td class="text-center">{{ $multimediat_intermed->phone }}</td>
                  <td class="text-center">{{ $multimediat_intermed->adresse }}</td>
                  <td class="text-center">{{ $multimediat_intermed->date_naissance }}</td>
                  <td class="text-center">{{ $multimediat_intermed->lieu_naissance }}</td>
                  <td class="text-center"><a data-toggle="modal" data-id="{{$multimediat_intermed->id}}" data-name="{{$multimediat_intermed->name}}" data-target="#modal-default-update-multimediat_intermed-{{ $multimediat_intermed->id }}"><i class="glyphicon glyphicon-edit"></i></a>
              
                    <form id="delete-form-{{$multimediat_intermed->id}}" method="post" action="{{ route('multimediat.destroy',$multimediat_intermed->id) }}" style="display:none">
                    {{csrf_field()}}
                    {{method_field('delete')}}
                    </form>
                  <a href="" onclick="
                    ...
                    event.preventDefault();document.getElementById('delete-form-{{$multimediat_intermed->id}}').submit();
    ...
    <!-- Fin du modal des edtions -->
      @foreach($multimediat_intermediare as $modal_intermediare)
        <div class="modal fade" id="modal-default-update-multimediat_intermed-{{ $modal_intermediare->id }}">
          <div class="modal-dialog modal-lg">
            <div class="modal-content">
              ...
              <form action="{{ route('multimediat.update',$modal_intermediare->id) }}" method="post">
              @csrf
              {{ method_field('PUT') }}
                      ...
                      <p>
                        <label for="slug">Niveau</label>
                          <select value="{{ old('niveau')  }}" class="form-control @error('niveau') is-invalid @enderror" id="niveau" name="niveau" placeholder="">
                            <option value="4" @if($modal_intermediare->module == 5) checked @endif>Niveau Initiale</option>
                            <option value="5">Niveau Intermediare</option>
                            <option value="6">Niveau Avance</option>
                          </select>
                          @error('niveau')
