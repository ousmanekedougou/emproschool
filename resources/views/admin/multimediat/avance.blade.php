...
@section('main-content')
    ...
    <section class="content">
      ...
      <div class="">
        <div class="">
          <h3 class="box-title">Multimediat Niveau Avancer</h3>
          ...
          <div class="tab-content">
            <div class="active tab-pane" id="activity">
              <table id="example1" class="table text-center table-bordered table-striped">
                ...
                <tbody>
                  @foreach($multimediat_avancer as $avancer)
                  <tr>
                  ...
                  <td class="text-center"><a data-toggle="modal" data-id="{{$avancer->id}}" data-name="{{$avancer->name}}" data-target="#modal-default-update-avancer-{{ $avancer->id }}"><i class="glyphicon glyphicon-edit"></i></a>
                    ...
                    <form id="delete-form-{{$avancer->id}}" method="post" action="{{ route('multimediat.destroy',$avancer->id) }}" style="display:none">
                    {{csrf_field()}}
                    {{method_field('delete')}}
                    </form>
                  <a href="" onclick="
    ...
    <!-- Fin du modal des edtions -->
      @foreach($multimediat_avancer as $modal_avancer)
        <div class="modal fade" id="modal-default-update-avancer-{{ $modal_avancer->id }}">
          <div class="modal-dialog modal-lg">
            <div class="modal-content">
              ...
              <form action="{{ route('multimediat.update',$modal_avancer->id) }}" method="post">
              @csrf
              {{ method_field('PUT') }}
                      ...
                      <p>
                        <label for="slug">Niveau</label>
                          <select value="{{ old('niveau')  }}" class="form-control @error('niveau') is-invalid @enderror" id="niveau" name="niveau" placeholder="">
                            <option value="4" @if($modal_avancer->module == 6) checked @endif>Niveau initiale</option>
